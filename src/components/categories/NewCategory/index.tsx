...
export default function NewCategory({ dialogOpen, setDialogOpen }: NewCategoryProps) {
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogTrigger asChild>
        <Button className="border border-light-secondary bg-primary text-xs">
          Categoria <Plus />
        </Button>
      </DialogTrigger>
      ...
      <DialogContent className="flex flex-col items-start sm:max-w-md bg-light-background">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2 w-full">
          {/* Botão e diálogo do IconPicker controlados aqui */}
          <div className="flex items-center gap-2">
            ...
            {/* Mostra o nome do ícone selecionado (opcional) */}
            {iconName && (
